...
/** Default local Ollama model + base URL for the desktop (offline) path. */
...
/**
 * Local Ollama path (Desktop Edition default) — OpenAI-compatible API exposed by
 * a local `ollama serve` at OLLAMA_BASE_URL (default http://localhost:11434).
 * No API key, no network egress, no marginal cost. Selected after the direct
 * cloud SDKs but before the Replit proxy, so a clean desktop with neither
 * ANTHROPIC_API_KEY nor OPENAI_API_KEY reasons fully offline.
 */
async function callOllama(
  ...
  const startedAt = Date.now();
  const model = process.env.JARVIS_OLLAMA_MODEL?.trim() || DEFAULT_OLLAMA_MODEL;
  try {
    const OpenAI = (await import("openai")).default;
    const client = new OpenAI({
    ...
    return {
      ...
      inputTokens,
      outputTokens,
      costMicros: 0, // local inference — no marginal cost
      latencyMs: Date.now() - startedAt,
      model,
      error: null,
    };
...
export async function callModel(
  ...
  // Desktop Edition default: a local Ollama server. OLLAMA_BASE_URL is honored
  // if set; otherwise we fall back to the conventional localhost endpoint so a
  // clean desktop install reasons offline with zero configuration.
  const ollamaBaseUrl =
    process.env.OLLAMA_BASE_URL?.trim() ||
    process.env.JARVIS_OLLAMA_BASE_URL?.trim() ||
    DEFAULT_OLLAMA_BASE_URL;
  if (ollamaBaseUrl) {
    return callOllama(input, ollamaBaseUrl);
  }
