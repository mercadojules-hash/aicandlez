...
// PortalExchangeConnectModal removed on admin path (Phase 1 graduation).
...
// Admin shell does NOT depend on PaperTradesProvider. We re-shape real
// operator telemetry (Kraken balances + admin/positions + admin/closed-trades)
// into the same `{ stats, open, history, openTrade }` contract via
// `useAdminPaperStub` below so the existing panel internals
// (AccountStatusStrip, PortfolioIntelligence, RecentExits, ExecutionAwareness,
// OpportunityCard "recently traded" lookup) continue to compile and render
...
const T = {
  ...
  NEON:        "#66FF66",
  // CONVICTION_V2 (2026-05-26): glow alpha boosted 0.45 → 0.70 to
  // mirror PortalCustomerShell. Operator and customer shells must read
  // at identical neon intensity so the brand never feels "two tones".
...
/* ──────────────────────────────────────────────────────────────────────── */
/* useAdminTelemetry — REAL operator data sources                           */
/* ──────────────────────────────────────────────────────────────────────── */
/* Coalesces three already-existing operator endpoints into one shape:      */
/*   • GET /api/exchange/balances    → live Kraken USD/BTC/ETH/SOL          */
/*   • GET /api/admin/positions      → all open positions platform-wide    */
/*   • GET /api/admin/closed-trades  → recent closed + summary             */
/* No new server routes added in Phase 1. All endpoints already require    */
/* admin/super-admin role server-side.                                      */

interface AdminBalances {
  source:   "live" | "cached" | "stale-error" | "error" | "standby" | "simulation";
  exchange: string;
...
interface AdminPositionsResp { positions: AdminPosition[]; count: number; timestamp: number; }
interface AdminClosedTrade {
  id:               string | number;
  user_id:          string | null;
  user_email:       string | null;
  symbol:           string;
...
interface AdminClosedResp {
  trades:    AdminClosedTrade[];
  summary:   { total: number; wins: number; losses: number; total_pnl: number };
  timestamp: number;
}

const adminTelemetryQ = { refetchInterval: 5_000, refetchOnWindowFocus: false, staleTime: 0 } as const;

function useAdminTelemetry() {
  const balancesQ = useQuery<AdminBalances>({
    queryKey: ["admin-portal", "kraken-balances"],
    queryFn:  () => authFetch(`${apiBaseUrl}/api/exchange/balances`, { credentials: "include" })
      .then((r) => r.json() as Promise<AdminBalances>),
    ...adminTelemetryQ,
  });
  const positionsQ = useQuery<AdminPositionsResp>({
    queryKey: ["admin-portal", "positions"],
    queryFn:  () => authFetch(`${apiBaseUrl}/api/admin/positions?limit=200`, { credentials: "include" })
      .then((r) => r.json() as Promise<AdminPositionsResp>),
    ...adminTelemetryQ,
  });
  const closedQ = useQuery<AdminClosedResp>({
    queryKey: ["admin-portal", "closed-trades"],
    queryFn:  () => authFetch(`${apiBaseUrl}/api/admin/closed-trades?limit=50`, { credentials: "include" })
      .then((r) => r.json() as Promise<AdminClosedResp>),
    ...adminTelemetryQ,
  });
...
/* ──────────────────────────────────────────────────────────────────────── */
/* useAdminPaperStub — re-shapes admin telemetry as PaperTrades contract.   */
/* ──────────────────────────────────────────────────────────────────────── */
/* Lets the unchanged panel internals (AccountStatusStrip, Portfolio-       */
/* Intelligence, RecentExits, ExecutionAwareness, OpportunityCard) read     */
/* `{ stats, open, history, openTrade }` without a PaperTradesProvider.    */
/* Real-money values flow through; the `openTrade` op is a no-op since      */
/* Phase 1 explicitly does NOT route live execution from the matrix.        */

interface AdminStubCtx {
  stats:      PaperStats;
  open:       PaperTrade[];
...
function AdminTelemetryProvider({ children }: { children: ReactNode }) {
  const t = useAdminTelemetry();
  const value = useMemo<AdminStubCtx>(() => {
    ...
    const mapReason = (r: string | null): "TP" | "SL" | "MANUAL" => {
      ...
      if (s === "TARGET" || s === "TP" || s === "TAKE_PROFIT") return "TP";
      if (s === "STOP"   || s === "SL" || s === "STOP_LOSS")   return "SL";
      return "MANUAL";
    };
    const history: ClosedPaperTrade[] = t.closed.map((c, i) => {
      const side: "LONG" | "SHORT" = (String(c.side).toUpperCase() === "SELL" || String(c.side).toUpperCase() === "SHORT") ? "SHORT" : "LONG";
      const entry = Number(c.entry_price) || 0;
      const exit  = Number(c.exit_price)  || 0;
      const size  = Number(c.size_usd) || 0;
      const at    = Number(c.exit_time) || Date.now();
      return {
        id:       String(c.id ?? `cls-${i}`),
        symbol:   c.symbol,
        display:  c.symbol,
        side,
        entry,
        ...
        openedAt: at,
        closedAt: at,
        pnl:      Number(c.realized_pnl) || 0,
        pnlPct:   Number(c.realized_pnl_pct) || 0,
        reason:   mapReason(c.close_reason),
      };
    });
    const wins   = t.summary.wins;
    ...
    for (const h of history) {
      if (h.closedAt >= startOfDay.getTime()) todayPnl += h.pnl;
    }
    let bestPnl = 0;
    let bestSymbol: string | null = null;
    for (const h of history) {
      if (h.pnl > bestPnl) { bestPnl = h.pnl; bestSymbol = h.symbol; }
    }
...
/* ──────────────────────────────────────────────────────────────────────── */
/* Phase2PlaceholderModal — admin BUY/SELL click sink (Phase 1)             */
/* ──────────────────────────────────────────────────────────────────────── */
/* Phase 1 explicitly DEFERS one-click Kraken execution from the matrix.   */
/* Every BUY/SELL click on the admin shell opens this modal instead of     */
/* routing to placeLiveAutoOrder. Phase 2 will replace this with a         */
/* confirm-dialog + notional-cap + cooldown + kill-switch flow.            */

function Phase2PlaceholderModal({ open, intent, onClose }: {
  open: boolean;
  intent: { symbol: string; side: "LONG" | "SHORT" } | null;
  onClose: () => void;
}) {
  ...
  return (
    <div
      ...
      style={{
        ...
        padding: 24, fontFamily: T.FONT_MONO,
      }}
    >
      <div
      ...
      >
        ...
        <h2 style={{
          ...
          letterSpacing: "-0.02em", color: T.TEXT_0,
        }}>
        ...
        <p style={{ margin: "0 0 22px 0", fontSize: 11, lineHeight: 1.55, color: T.TEXT_2, letterSpacing: "0.04em" }}>
          ...
          server-side `placeLiveAutoOrderForUser` engine on the operator path.
        </p>
...
const OperatorPulseRibbon = memo(function OperatorPulseRibbon({
  ...
  // Phase 1 admin graduation: ribbon plan slot reads as operator status.
  // Customer plan tiers are not meaningful on the admin terminal.
  void plan;
  const planLabel = "OPERATOR · LIVE · KRAKEN";
...
const OpportunityCard = memo(function OpportunityCard({ opp, onQueue, idx = 0, now }: {
  ...
  // only, no animation).
  const { history: paperHistory } = useAdminPaperStub();
...
const OpportunityMatrix = memo(function OpportunityMatrix({
  ...
  // CONVICTION_V2 (2026-05-26): mirror PortalCustomerShell — partition
  // by ASSET CLASS (Majors / Alts) instead of DIRECTION (Longs /
  // Shorts). Operator parity with the customer shell is a non-negotiable
  // for this surface: if the two shells render the same opportunity
  // pool with different framings the operator and the customer end up
  // looking at fundamentally different products. Direction is preserved
  // as an inline LONG/SHORT/FLAT badge on each OpportunityCard, so
  // polarity is per-row, not per-column. Both columns share neon accent
    ...
    <div style={{
      ...
      >
        ...
        {/* Pass 7P — EVALUATING tier render block DELETED. All
            ...
            from the unified asset-class-partitioned derivation
            (CONVICTION_V2: filteredMajors / filteredAlts). Removed:
            dim/desaturate wrapper, EVAL header, pointer-events gate.
            ~70 lines of two-tier visual hierarchy collapsed into one. */}
      </div>
    </div>
  );
});
...
const PortfolioIntelligence = memo(function PortfolioIntelligence({ now }: { now: number }) {
  const { stats, open, history } = useAdminPaperStub();
...
const RecentExits = memo(function RecentExits({ now }: { now: number }) {
  const { history } = useAdminPaperStub();
        ...
        <div style={{
          ...
          <div style={{
            ...
            <span style={{
              ...
              {engineLabel}  ·  LIVE AI TRADING IS CURRENTLY DISABLED
            </span>
          </div>
          <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <span style={{
            ...
            AI IS EXECUTING LIVE TRADES  ·  OPEN {openPaper}
          </span>
        </div>
...
function AdminPortalShellInner() {
  ...
  // Phase 1 admin: plan tier is irrelevant on the operator terminal.
  // Fixed "pro" sentinel keeps the `Plan` type contract for downstream
  // props (OperatorPulseRibbon overrides its own label).
  const plan: Plan = "pro";
  ...
  const { stats: paperStats, openTrade, open: openTrades } = useAdminPaperStub();
  ...
  // Phase 1 admin: customer-modal state (account / upgrade / disclaimer /
  // exchange-connect) is gone. Only the Phase 2 placeholder modal remains.
  const [phase2Intent, setPhase2Intent] = useState<{ symbol: string; side: "LONG" | "SHORT" } | null>(null);
  ...
  // Phase 1 admin: matrix BUY/SELL clicks do NOT route to live execution.
  // Each click opens the Phase 2 placeholder modal. AI auto-execution
  // continues to flow through the existing server-side
  // `placeLiveAutoOrderForUser` engine path — this stub only governs the
  // human-click matrix surface, which is the new attack vector that needs
  // the Phase 2 safety layer (confirm dialog, notional caps, cooldown,
  // kill switch, audit) before going live.
  const queuePaper = useCallback((opp: OpportunityVM, side: "LONG" | "SHORT") => {
    setPhase2Intent({ symbol: opp.display ?? opp.symbol, side });
  }, []);
  void openTrade; // openTrade is a no-op on admin path; reference retained for shape.
  ...
  return (
    <div className="cd-portal-root" style={{
      ...
      <OperatorPulseRibbon
        ...
        now={nowShell}
      />
      {/* Phase 1 admin: PaperModeBanner removed. Admin path is LIVE-only. */}
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, justifyContent: "flex-end", alignItems: "center" }}>
              {/* Phase 1 admin: ACCOUNT, CONNECT EXCHANGE, UPGRADE, DISCLAIMER
                  removed. Admin terminal is live-only; account & exchange
                  state live in the AdminTopTelemetryBar and the ACCOUNT
                  STATUS strip below. ExchangeStatusBadge replaced with a
                  static neon "KRAKEN LIVE CONNECTED" chip. */}
              <KrakenLiveConnectedChip />
            </div>
      ...
      {/* Phase 1 admin: Upgrade/Account/Disclaimer/PortalExchangeConnect
          modals removed. Only the Phase 2 placeholder modal remains —
          it intercepts matrix BUY/SELL clicks until the live-execution
          safety layer ships. */}
      <Phase2PlaceholderModal
        open={phase2Intent !== null}
        intent={phase2Intent}
        onClose={() => setPhase2Intent(null)}
      />
    </div>
  );
}

/* KRAKEN LIVE CONNECTED — static neon status chip for the admin header.    */
/* Replaces ExchangeStatusBadge on the admin surface. No interactivity.     */
function KrakenLiveConnectedChip() {
  return (
    <div
      title="Operator Kraken account · server-side keys · live"
      style={{
        display: "inline-flex", alignItems: "center", gap: 8,
        padding: "7px 14px",
