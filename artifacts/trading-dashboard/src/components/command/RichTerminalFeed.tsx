...
const STAGE_CFG: Record<string, { label: string; color: string; icon: string }> = {
  ...
  EXECUTION:  { label: "TRADE",   color: "#00cc6a", icon: "→" },
  ROUTING:    { label: "ROUTE",   color: "#00ccff", icon: "⟳" },
  BLOCKED:    { label: "BLOCKED", color: "#ff3355", icon: "✗" },
  MONITORING: { label: "AI",      color: "#0099ff", icon: "◆" },
  VALIDATING: { label: "VALID",   color: "#ffaa00", icon: "⟳" },
  SCANNING:   { label: "SIGNAL",  color: "#7b68ee", icon: "◈" },
};

const CAT_CFG: Record<Category, { color: string }> = {
  ALL:    { color: "#4a8fa8" },
  SIGNAL: { color: "#cc55ff" },
  TRADE:  { color: "#00ff8a" },
  ALERT:  { color: "#ff6600" },
...
export function RichTerminalFeed({ engine }: Props) {
  ...
  return (
    <div className="terminal-card flex flex-col h-full">
        ...
        {TABS.map(tab => (
          <button key={tab.key}
            ...
            style={filter === tab.key
              ? {
                  background: `${CAT_CFG[tab.key].color}14`,
                  color: CAT_CFG[tab.key].color,
                  border: `1px solid ${CAT_CFG[tab.key].color}28`,
                }
      ...
      {/* Feed rows — tighter density, ~20 visible */}
      <div ref={scrollRef} className="feed-scroll flex-1 overflow-y-auto" style={{ maxHeight: 580 }}>
        {visible.length === 0 ? (
            ...
            return (
              <div key={s.id}
                style={{
                  ...
                  borderLeft: `2px solid ${stageCfg.color}${isFilled ? "80" : "30"}`,
                  background: isBlocked ? "#ff33550a"
                    : isFilled  ? "#00ff8a06"
                    : i === 0   ? "#030303"
                    : "transparent",
                ...
                <div className="flex items-center gap-2 px-2.5 py-1.5">
                  ...
                  <span className="text-[11px] font-bold font-mono flex-shrink-0"
                    style={{ color: decColor, textShadow: s.decision !== "HOLD" ? `0 0 6px ${decColor}40` : undefined }}>
                    {s.decision}
                  </span>
                  ...
                  <span className="text-[9px] font-mono truncate flex-1 font-medium"
                    style={{ color: isBlocked ? "#ff335570" : "#9FB3C8" }}>
                ...
                {/* Mini conf bar */}
                <div style={{ paddingLeft: 62, paddingRight: 10, paddingBottom: 4 }}>
        ...
        <div className="flex items-center gap-2 px-3 py-2" style={{ borderLeft: "2px solid #0d0d0d" }}>
          ...
          <span className="text-[8px] font-mono font-medium" style={{ color: "#9FB3C8" }}>
            {engine?.running
              ? `ENGINE RUNNING · NEXT ${nextTick}s · SIGNALS: ${engine.signalsGenerated ?? 0}`
              : "ENGINE IDLE"}
          </span>
