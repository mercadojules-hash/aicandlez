...
function MetricCell({ label, value, delta, deltaUp, color = "#00f0ff" }: Metric) {
  return (
    <div className="flex flex-col gap-0.5 px-3 shrink-0 min-w-[80px]">
      <div className="text-[15px] font-bold font-mono tabular-nums leading-none" style={{ color }}>
        {value}
      </div>
      {delta && (
        <div className="text-[9px] font-mono font-semibold" style={{ color: deltaUp ? "#00ff8a" : "#ff3355" }}>
          {deltaUp ? "↑" : "↓"} {delta}
        </div>
      )}
      <div className="text-[9px] font-mono uppercase tracking-[0.08em] leading-none font-medium mt-0.5"
        style={{ color: "#9FB3C8" }}>
        {label}
      </div>
    </div>
...
function Section({
  ...
  return (
    <div className="flex gap-0 items-stretch shrink-0" style={{ borderRight: "1px solid #181818" }}>
      <div
        className="flex items-center shrink-0 px-2"
        style={{ background: `${color}08`, borderRight: `1px solid ${color}20`, writingMode: "vertical-rl" }}
      >
        <span className="text-[7px] font-bold font-mono tracking-[0.2em] rotate-180"
          style={{ color: `${color}90` }}>
          {title}
        </span>
      </div>
      <div className="flex items-center py-2" style={{ gap: 0 }}>
        {children}
      </div>
    </div>
  );
...
export function PlatformTelemetryBar({ engine, settings, trades, exchangeStatus, feeSummary }: Props) {
  ...
  return (
    <div
      ...
      style={{ background: "#000000", borderBottom: "1px solid #181818" }}
    >
      ...
      <Section title="TRADING ACTIVITY" color="#00ff8a">
        <MetricCell label="OPEN POSITIONS" value={open.length.toString()} color={open.length > 0 ? "#00f0ff" : "#C7D4E2"} />
      ...
      <Section title="SYSTEM HEALTH" color="#00eeff">
        ...
        <MetricCell label="QUEUE"        value={queueDepth.toString()}         color={queueDepth < 5 ? "#00ff8a" : "#ffaa00"} />
