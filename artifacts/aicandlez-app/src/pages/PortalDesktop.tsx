...
function TopBar({
  ...
  return (
    <div style={{
      ...
      <a
        href={`${basePath}/settings/exchanges`}
...
function LiveExecutionBar({
  ...
  const handle = () => {
    ...
    if (exchangeLocked) { window.location.assign(`${basePath}/settings/exchanges`); return; }
    setArmed(a => !a);
  };
...
function PortalInner() {
  ...
  const { data: engine } = useQuery({
    ...
    queryFn:  () => j<EngineStatus>(`${apiBaseUrl}/api/engine/status`),
    ...Q_MEDIUM,
  });
