...
- `realized_pnl_pct` (and `realized_pnl`) is **P&L-on-notional AFTER fees AND
  ...
  Effective notional << `size_usd`: per-account multiplier ≈ **0.37** (info,
  ~$10 effective on $20 nominal) vs **0.72** (teedelgado). Cause: Coinbase
  min-increment rounding on ~$20 orders of $2–$5 coins.
...
## Counterfactual: SL2/TP5/Trail3 vs current SL2/TP4/Trail1.5 (post-deploy prod)
...
- Evidence = MFE (peak favorable excursion) reconstructed from 1m Coinbase
  ...
  trades barely move up: only ~15% of teedelgado trades peaked ≥4%, ~4% peaked
  ≥5%, and ~44% never reached +1.5%; info: 0 of 8 reached ≥4%.
