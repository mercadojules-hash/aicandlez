// ── backfill-open-order-ids ──────────────────────────────────────────────────
//
// Sibling reconciliation script to `backfill-close-order-ids`.
//
// Some very old live trades (opened during the dry-run rollout or on partial
// failure paths) have a NULL `exchange_order_id` even though the broker
// actually shows a matching open-side fill from that period. This script
// pulls each affected user's broker order history (Alpaca /v2/orders,
// Kraken ClosedOrders) and back-fills `exchange_order_id` on `sim_trades`
// rows where a unique match can be found, so trade history can show both
// the open-side and close-side broker order IDs uniformly for every
// historical live trade.
//
// Idempotent: only touches rows where `exchange_order_id IS NULL`
// AND `exchange IS NOT NULL`. Re-running after a partial run is safe.
//
// Run with:
//   pnpm --filter @workspace/scripts run backfill-open-order-ids
//
...
// For open-side matching we key off `opentm` (submission time), not `closetm`,
// because the trade's `entryTime` reflects when the position was opened.
async function fetchKrakenOpenOrders(
  creds: ExchangeCredentials,
  sinceMs: number,
): Promise<BrokerOrder[]> {
  ...
  for (let page = 0; page < 20; page++) {
    ...
    for (const [id, o] of entries) {
      ...
      const openSec   = o.opentm ?? o.closetm ?? 0;
      if (!(filledQty > 0) || !(openSec > 0)) continue;
      all.push({
        exchangeOrderId: id,
        symbol:          KRAKEN_SYMBOL_MAP[o.descr.pair] ?? o.descr.pair,
        side:            o.descr.type === "buy" ? "buy" : "sell",
        filledQty,
        filledAt:        openSec * 1000,
      });
    }
    ofs += entries.length;
...
const TIME_WINDOW_MS    = 10 * 60_000; // ±10 minutes around recorded entryTime

function openSideFor(tradeSide: string): "buy" | "sell" {
  return tradeSide.toUpperCase() === "BUY" ? "buy" : "sell";
}

function isCandidate(trade: SimTrade, order: BrokerOrder): boolean {
  if (order.symbol !== trade.symbol) return false;
  if (order.side !== openSideFor(trade.side)) return false;
  ...
  if (Math.abs(order.filledAt - trade.entryTime) > TIME_WINDOW_MS) return false;
  return true;
}
...
export async function runOpenOrderIdBackfill(): Promise<BackfillSummary> {
  ...
  const candidates = await db
    ...
    .from(simTradesTable)
    .where(and(
      isNull(simTradesTable.exchangeOrderId),
  ...
  for (const [groupKey, tradesUnsorted] of grouped) {
    ...
    // Deterministic iteration: oldest entries first, tie-broken on id.
    const trades = [...tradesUnsorted].sort((a, b) =>
      a.entryTime - b.entryTime || (a.id < b.id ? -1 : a.id > b.id ? 1 : 0));
    if (exchange !== "Alpaca" && exchange !== "Kraken") {
      console.log(`[backfill-open] skip ${trades.length} rows for ${exchange} (no historical-orders adapter)`);
    ...
    if (!conn) {
      console.warn(`[backfill-open] user=${userId.slice(0, 8)}… exchange=${exchange}: no stored connection, skipping ${trades.length} rows`);
      for (const _ of trades) { totalSkipped++; bumpStat(exchange, "skipped"); }
      continue;
    }
    ...
    if (!creds) {
      console.warn(`[backfill-open] user=${userId.slice(0, 8)}… exchange=${exchange}: decryptBlob failed, skipping ${trades.length} rows`);
      for (const _ of trades) { totalErrored++; bumpStat(exchange, "errored"); }
      continue;
    }

    // For open-side reconciliation the relevant anchor is `entryTime`, not
    // `exitTime`. Pull broker history back far enough to cover the earliest
    // recorded entry minus the tolerance window.
    const earliestEntry = trades.reduce((m, t) => Math.min(m, t.entryTime), Number.POSITIVE_INFINITY);
    const sinceMs       = Math.max(0, earliestEntry - TIME_WINDOW_MS);

    let history: BrokerOrder[];
    try {
      if (exchange === "Alpaca") {
        // Alpaca's filled_at is the open-side fill timestamp for an
        // open-side order, so the same fetch function works for both
        // sides — only the matching anchor differs.
        const res = await fetchAlpacaClosedOrders(creds, sinceMs);
        history = res.orders;
        if (res.truncated) {
          console.warn(`[backfill-open] user=${userId.slice(0, 8)}… exchange=Alpaca: order history TRUNCATED at page cap — unmatched counts for this user may not be trustworthy. Re-run after raising ALPACA_PAGE_CAP.`);
        }
      } else {
        history = await fetchKrakenOpenOrders(creds, sinceMs);
      }
    } catch (err) {
      console.error(`[backfill-open] user=${userId.slice(0, 8)}… exchange=${exchange}: order-history fetch failed:`, (err as Error).message);
      for (const _ of trades) { totalErrored++; bumpStat(exchange, "errored"); }
      continue;
    }
    ...
    console.log(`[backfill-open] user=${userId.slice(0, 8)}… exchange=${exchange}: fetched ${orders.length} historical orders since ${new Date(sinceMs).toISOString()}`);

    // Strict 1:1 matching with ambiguity detection.
    //   1. For each trade, collect ALL broker orders that fall inside the
    //      tolerance window (symbol + same side + qty±1% + ±10min of entry).
    ...
    for (const trade of trades) {
      ...
      if (matches.length > 1) {
        ...
        console.warn(`[backfill-open] ambiguous trade=${trade.id} symbol=${trade.symbol} qty=${trade.quantity} → ${matches.length} candidate orders (${matches.map(m => m.exchangeOrderId).join(", ")}); skipping`);
        continue;
      }
      ...
      if (claimants.length > 1) {
        totalAmbiguous++; bumpStat(exchange, "ambiguous");
        console.warn(`[backfill-open] ambiguous trade=${trade.id} symbol=${trade.symbol} qty=${trade.quantity} → broker order ${order.exchangeOrderId} claimed by ${claimants.length} trades (${claimants.map(c => c.id).join(", ")}); skipping`);
        continue;
      }

      await db
        .update(simTradesTable)
        .set({ exchangeOrderId: order.exchangeOrderId })
        .where(and(
          eq(simTradesTable.id, trade.id),
          isNull(simTradesTable.exchangeOrderId),
        ));
      totalMatched++; bumpStat(exchange, "matched");
      console.log(`[backfill-open] matched trade=${trade.id} symbol=${trade.symbol} qty=${trade.quantity} → ${order.exchangeOrderId}`);
    }
  }

  console.log("\n[backfill-open] === summary ===");
